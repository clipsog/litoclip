...
router.get('/creator', requireAuth, requireCreator, (req, res) => {
  ...
  const activeCampaigns = db.prepare('SELECT COUNT(DISTINCT campaign_id) as c FROM campaign_joins WHERE user_id = ?').get(uid).c;
...
router.get('/creator/timeline', requireAuth, requireCreator, (req, res) => {
...
router.get('/creator/platforms', requireAuth, requireCreator, (req, res) => {
  const rows = db.prepare(`
...
router.get('/campaign/:campaignId', requireAuth, requireCreator, (req, res) => {
  const joined = db.prepare('SELECT 1 FROM campaign_joins WHERE user_id = ? AND campaign_id = ?').get(req.user.id, req.params.campaignId);
  if (!joined) return res.status(404).json({ error: 'Not found' });
  const rows = db.prepare('SELECT views, earnings FROM submissions WHERE campaign_id = ? AND user_id = ?').all(req.params.campaignId, req.user.id);
...
router.get('/leaderboard', requireAuth, requireCreator, (req, res) => {
