...
router.get('/my-position', requireAuth, requireCreator, (req, res) => {
  const metric = req.query.metric || 'views';
  const col = metric === 'earnings' ? 'earnings' : 'views';
  const myTotal = db.prepare(`SELECT COALESCE(SUM(${col}), 0) as total FROM submissions WHERE user_id = ?`).get(req.user.id).total;
