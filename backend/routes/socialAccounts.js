...
router.get('/', requireAuth, requireCreator, async (req, res) => {
  const rows = await db.prepare('SELECT id, platform, handle, status, verification_code FROM social_accounts WHERE user_id = ? ORDER BY created_at DESC').all(req.user.id);
  res.json(rows.map(r => ({
    id: r.id,
    platform: r.platform,
    handle: r.handle,
...
router.post('/generate-code', requireAuth, requireCreator, async (req, res) => {
...
router.post('/verify', requireAuth, requireCreator, async (req, res) => {
  const { accountId, code, skipApiCheck } = req.body || {};
  if (!accountId || !code) return res.status(400).json({ error: 'accountId and code required' });
  const row = await db.prepare('SELECT id, verification_code FROM social_accounts WHERE id = ? AND user_id = ?').get(accountId, req.user.id);
  if (!row) return res.status(404).json({ error: 'Account not found' });
  if (row.verification_code !== code) return res.status(400).json({ error: 'Invalid code' });
  await db.prepare('UPDATE social_accounts SET status = ?, verification_code = NULL WHERE id = ?').run('verified', accountId);
  res.json({ ok: true });
});

// DELETE /api/social-accounts/:id
router.delete('/:id', requireAuth, requireCreator, async (req, res) => {
  const result = await db.prepare('DELETE FROM social_accounts WHERE id = ? AND user_id = ?').run(req.params.id, req.user.id);
