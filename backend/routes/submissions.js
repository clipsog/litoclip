...
router.post('/', requireAuth, requireCreator, async (req, res) => {
  ...
  const joined = await db.prepare('SELECT 1 FROM campaign_joins WHERE user_id = ? AND campaign_id = ?').get(req.user.id, campaignId);
  ...
  const row = await db.prepare('SELECT id, user_id, campaign_id, platform, post_url, status, views, likes, earnings, created_at FROM submissions WHERE id = ?').get(id);
  res.status(201).json({
    id: row.id,
    userId: row.user_id,
...
router.post('/batch', requireAuth, requireCreator, async (req, res) => {
  ...
  for (const s of items) {
    ...
    await db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `).run(id, req.user.id, campaignId, platform, postUrl);
    const row = await db.prepare('SELECT id, platform, post_url, status, views, likes, earnings, created_at FROM submissions WHERE id = ?').get(id);
    created.push(row);
  }
...
router.post('/:id/refresh-engagement', requireAuth, requireCreator, async (req, res) => {
  const row = await db.prepare('SELECT id FROM submissions WHERE id = ? AND user_id = ?').get(req.params.id, req.user.id);
